...
\begin{tikzpicture}[node distance=2.8cm,
   ...
   \draw [->] ( $ (pop6.west)$) --+(-3,0) node[above]{$\Delta$}-| ($(accept.south)$);
   ...
   \draw [->] ( $ (pop8.south)$) -| node[left]{$\Delta$} ($(accept.north)$);
\end{tikzpicture}
\end{center}
\end{figure}
\clearpage
...
$READ_1$&ACCEPT&$\Lambda$&$\Lambda$&-&2\\ \hline
...
$READ_3$&$ACCEPT$&$\Lambda$&$\Lambda$&-&8\\ \hline
\end{tabular}
\end{center}
\end{figure}
%tape one

...
\begin{tikzpicture}[node distance=2.8cm,
   ...
   \draw[->]  ( $ (read1.south)$) -++(-2,0) -++(-2,-0.9);
   \draw[->]  ( $ (read1.south)$) -++(2,0) -++(2,-0.9);
    \draw[->]  ( $ (pop6.south)$) -| ($(push10.north)$);
    \draw[->]  ( $ (pop7.south)$) -| ($(push11.north)$);
    \draw[->]  ( $ (push10.south)$) -| ($(pop8.west)$);
    \draw[->]  ( $ (push11.south)$) -| ($(pop8.east)$);
   \draw[->]  ( $ (pop8.south)$) -| ($(read2.north)$);
    \draw[->]  ( $ (read2.south)$) -| ($(pop9.north)$);
     \draw[->]  ( $ (pop9.south)$) -| ($(accept.north)$);
     \draw[->]  ( $ (read2.west)$)-++(-2,0) -++(-2,8.5)-++(-0.9,8.5);
     \draw[->]  ( $ (read2.east)$)-++( 2,0) -++( 2,8.5) -++(0.9,8.5);
\end{tikzpicture}
\end{center}
\end{figure}
\clearpage
